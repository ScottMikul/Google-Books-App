...
export function Search() {
    ...
    function searchGoogleBooks() {
        ...
        axios.get("https://www.googleapis.com/books/v1/volumes?q=\"" + searchText + "\"").then(
            res => {
                const booksArr = res.data.items.slice(0, 5).map(
                    item => {
                        let thumbnail;
                        if (item.volumeInfo.imageLinks) {
                            thumbnail = <img src={item.volumeInfo.imageLinks.thumbnail}></img>
                        }
                        console.log({thumbnail:item.volumeInfo.imageLinks.thumbnail, title:item.volumeInfo.title,
                        ...
                        return (
                            <div className="card m-2">
                                ...
                                <div></div>
                                <div className="btn btn-primary" onClick={()=>API.saveBook({thumbnail:item.volumeInfo.imageLinks.thumbnail, title:item.volumeInfo.title,
                                    authors:item.volumeInfo.authors,description:item.volumeInfo.description, link:item.volumeInfo.infoLink})}>
                                        Save
                                </div>
                                </div>
